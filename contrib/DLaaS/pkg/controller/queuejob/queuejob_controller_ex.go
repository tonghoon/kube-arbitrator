...
import (
	...
	respersistentvolume "github.com/kubernetes-sigs/kube-batch/contrib/DLaaS/pkg/controller/queuejobresources/persistentvolume" // PV
	respersistentvolumeclaim "github.com/kubernetes-sigs/kube-batch/contrib/DLaaS/pkg/controller/queuejobresources/persistentvolumeclaim" // PVC
	resnamespace "github.com/kubernetes-sigs/kube-batch/contrib/DLaaS/pkg/controller/queuejobresources/namespace" // NP
	ressecret "github.com/kubernetes-sigs/kube-batch/contrib/DLaaS/pkg/controller/queuejobresources/secret" // Secret
	resnetworkpolicy "github.com/kubernetes-sigs/kube-batch/contrib/DLaaS/pkg/controller/queuejobresources/networkpolicy" // NetworkPolicy
	resconfigmap "github.com/kubernetes-sigs/kube-batch/contrib/DLaaS/pkg/controller/queuejobresources/configmap" // ConfigMap
...
func (qjm *XController) chooseAgent(qj *arbv1.XQueueJob) string{
	...
	// Power of Two Choices: Packing? Spreading(load-balancing)?
...
func (qjm *XController) ScheduleNext() {
	...
	if qjm.isDispatcher {			// Dispatcher Mode
		...
		if agentId != "" {			// A proper agent is found.
			...
			glog.Infof("[Controller: Dispatcher Mode] Cannot find an Agent with enough Resources\n")
			go qjm.backoff(qj)
		}
	} else {						// Agent Mode
...
func (qjm *XController) UpdateQueueJobs() {
	...
	for _, newjob := range queueJobs {
		if !qjm.qjqueue.IfExist(newjob) {
			...
			// glog.Infof("[Tonghoon] QueueJob %s exists\n", newjob.Name)
		}
  }
}
...
func (cc *XController) syncQueueJob(qj *arbv1.XQueueJob) error {
	...
	if err != nil {
		if apierrors.IsNotFound(err) {
			if (cc.isDispatcher) {
				// glog.Infof("[Tonghoon] Delete XQJ\n")
				cc.Cleanup(qj)
...
func (cc *XController) manageQueueJob(qj *arbv1.XQueueJob) error {
	...
	startTime := time.Now()
	defer func() {
		glog.Infof("[Controller] Finished syncing queue job %q (%v)", qj.Name, time.Now().Sub(startTime))
	}()
