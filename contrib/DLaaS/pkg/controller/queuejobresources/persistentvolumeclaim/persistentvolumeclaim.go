...
func (qjrPod *QueueJobResPersistentVolumeClaim) GetAggregatedResources(job *arbv1.XQueueJob) *schedulerapi.Resource {
...
func (qjrPod *QueueJobResPersistentVolumeClaim) GetAggregatedResourcesByPriority(priority int, job *arbv1.XQueueJob) *schedulerapi.Resource {
...
func (qjrPod *QueueJobResPersistentVolumeClaim) UpdateQueueJobStatus(queuejob *arbv1.XQueueJob) error {
	return nil
}
...
func (qjrPersistentVolumeClaim *QueueJobResPersistentVolumeClaim) SyncQueueJob(queuejob *arbv1.XQueueJob, qjobRes *arbv1.XQueueJobResource) error {
	...
	if diff > 0 {
		...
		for i := 0; i < diff; i++ {
			go func() {
				defer wait.Done()
				err := qjrPersistentVolumeClaim.createPersistentVolumeClaimWithControllerRef(queuejob.Namespace, template, metav1.NewControllerRef(queuejob, queueJobKind))
...
func (qjrPersistentVolumeClaim *QueueJobResPersistentVolumeClaim) getPersistentVolumeClaimForQueueJob(j *arbv1.XQueueJob) ([]*v1.PersistentVolumeClaim, error) {
	persistentvolumeclaimlist, err := qjrPersistentVolumeClaim.clients.CoreV1().PersistentVolumeClaims(j.Namespace).List(metav1.ListOptions{})
	if err != nil {
		return nil, err
	}
...
func (qjrPersistentVolumeClaim *QueueJobResPersistentVolumeClaim) getPersistentVolumeClaimForQueueJobRes(qjobRes *arbv1.XQueueJobResource, j *arbv1.XQueueJob) ([]*v1.PersistentVolumeClaim, error) {
	...
	persistentvolumeclaims, err := qjrPersistentVolumeClaim.getPersistentVolumeClaimForQueueJob(j)
	if err != nil {
		return nil, err
	}
