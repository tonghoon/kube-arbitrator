...
func (qjrStatefulSet *QueueJobResStatefulSet) GetAggregatedResources(queueJob *arbv1.XQueueJob) *schedulerapi.Resource {
...
func (qjrStatefulSet *QueueJobResStatefulSet) GetAggregatedResourcesByPriority(priority int, queueJob *arbv1.XQueueJob) *schedulerapi.Resource {
	total := schedulerapi.EmptyResource()
	if queueJob.Spec.AggrResources.Items != nil {
		//calculate scaling
		for _, ar := range queueJob.Spec.AggrResources.Items {
